...
**Authored here, not sourced.** CC0 by construction and generated by
`scripts/build-figure.mjs` — run it after editing that file. It writes two:
`villager-low.glb` (~15 KB, 204 triangles, the boxes the game shipped with),
drawn at quality tier 1, and `villager.glb` (~34 KB, 464 triangles: a jaw, hair,
shoulders, a belt, knees and boots) at tier 2 — see *Models* in
`docs/specs/rendering.md`. Both keep the joint heights below.

The clause above is "CC0 only", and CC0 *humanoids* turn out to be harder to
come by than that makes it sound. What was checked, and why each was rejected:

| Source | Verdict |
|---|---|
| Khronos glTF Sample Assets (`CesiumMan`, `RiggedFigure`) | CC-BY 4.0, © Cesium — attribution required, so not CC0 |
| three.js examples (`RobotExpressive`) | genuinely CC0, and a robot |
| three.js examples (`Xbot`) | Mixamo, restrictive |
| Kenney, Quaternius character packs | genuinely CC0, but published as zips behind pages rather than as fetchable files |

The last row is the one worth revisiting: those packs are good and their licence
is right. **Swapping one in is a file copy, not a code change** — the loader in
`renderer/vegetation.ts` derives limbs from vertex *positions* against the joint
heights in `FIGURE`, so any humanoid in a roughly upright pose can take this
one's place. That is why the model is a real glTF rather than geometry built in
code.

No skinning, deliberately: a thousand `SkinnedMesh`es is not a budget this
project has. The walk cycle is a vertex-shader rotation about those joint
heights, so the whole population stays one instanced draw call.
